...
def sidebar(current_route: str) -> rx.Component:
    return rx.el.aside(
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    rx.icon("trending-up", class_name="h-6 w-6 text-blue-600"),
                ...
                class_name="flex items-center gap-3 px-2 py-8 mb-4 border-b border-gray-100",
            ),
            rx.el.nav(
                rx.el.div(
                    nav_item("Perfil", "user", "/app/perfil", current_route),
                    nav_item(
                        "Veículos", "truck", "/app/veiculos", current_route
                    ),
                    nav_item("Custos", "wallet", "/app/custos", current_route),
                    nav_item(
                        "Resultados",
                        "bar-chart-3",
                        "/app/resultados",
        ...
        class_name="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-200 z-50",
    )